...
namespace FEXCore::Context {
  ...
  struct AOTIRBinaryArray {
    uint64_t Count;
    uint64_t DataBase;
    AOTIRBinaryEntry Entries[0];
  ...
  struct Context {
    ...
    std::unordered_map<std::string, AOTIRBinaryArray *> AOTIRCache;
    std::function<int(const std::string&)> AOTIRLoader;
    std::unordered_map<std::string, std::map<uint64_t, AOTIRCacheEntry>> AOTIRCaptureCache;
