...
extern std::vector<uint64_t> ExternalLocations;
extern uint64_t ElfMinAddr;
extern uint64_t ElfMaxAddr;
...
int main(int argc, char **argv, char **const envp) {
  ...
  auto layout = Loader.File.GetLayout();
  ElfMinAddr = std::get<0>(layout) + Loader.DB.GetElfBase();
  ElfMaxAddr = std::get<2>(layout) + Loader.DB.GetElfBase();

  std::vector<uint64_t> Entrypoints;
  Loader.GetInitLocations(&Entrypoints);
  Loader.GetSymbols(&Entrypoints);

FEXCore::Context::AddNamedRegion(CTX, ElfMinAddr, ElfMaxAddr-ElfMinAddr, 0, Program);

  printf("%zu entrypoints %lx-%lx\n\n", Entrypoints.size(), ElfMinAddr, ElfMaxAddr);

  std::set<uint64_t> Compiled;
  do {
    ExternalLocations.clear();
    for (auto Entrypoint: Entrypoints) {
      if (!Compiled.contains(Entrypoint) && Entrypoint > ElfMinAddr && Entrypoint < ElfMaxAddr) {
        Compiled.insert(Entrypoint);
        //printf("\rCompiling %lx", Entrypoint);
        FEXCore::Context::Debug::CompileRIP(CTX, Entrypoint);
      }
    }
    Entrypoints = ExternalLocations;
  } while (ExternalLocations.size());
  printf("\rAll done, total %lu superblocks\n", Compiled.size());
