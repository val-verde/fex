    ...
    for (auto [CodeNode, IROp] : CurrentIR.GetCode(BlockNode)) {
      switch (IROp->Op) {
        case IR::OP_CONDJUMP: {
          ...
          OrderedNode *TrueTargetNode = CurrentIR.GetNode(Op->Header.Args[3]);
          OrderedNode *FalseTargetNode = CurrentIR.GetNode(Op->Header.Args[4]);

          CurrentBlock->Successors.emplace_back(TrueTargetNode);
          CurrentBlock->Successors.emplace_back(FalseTargetNode);

          {
            auto Block = &OffsetToBlockMap.try_emplace(Op->Header.Args[3].ID()).first->second;
            Block->Predecessors.emplace_back(BlockNode);
          }

          {
            auto Block = &OffsetToBlockMap.try_emplace(Op->Header.Args[4].ID()).first->second;
            Block->Predecessors.emplace_back(BlockNode);
          }
